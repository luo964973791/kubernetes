...
alias pcap_tool='bash -c "nsenter --target \$(docker inspect --format '\''{{.State.Pid}}'\'' \$0) --net tcpdump -i any -nnll -s0 -A port \$(docker inspect \$0 | grep ExposedPorts -A 3 | grep -o '\''[0-9]\+/tcp'\'' | head -n1 | cut -d '\''/'\'' -f1)"'
