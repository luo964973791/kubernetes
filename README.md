...
alias pcap_tool='bash -c "command -v nsenter >/dev/null 2>&1 || { (command -v yum >/dev/null 2>&1 && yum install -y util-linux) || (command -v apt-get >/dev/null 2>&1 && apt-get update && apt-get install -y util-linux); }; pid=\$(docker inspect --format '\''{{.State.Pid}}'\'' \$0 2>/dev/null); if [ -z \"\$pid\" ] || [ \"\$pid\" = \"0\" ]; then echo \"Container \$0 not running or not found\"; else port=\$(docker inspect \$0 | grep ExposedPorts -A 3 | grep -o '\''[0-9]\+/tcp'\'' | head -n1 | cut -d '\''/'\'' -f1); nsenter --target \$pid --net tcpdump -i any -nnll -s0 -A port \$port; fi"'
