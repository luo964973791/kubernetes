...
alias pcap_tool='bash -c "nsenter --target \$(docker inspect --format '\''{{.State.Pid}}'\'' \$0) --net tcpdump -i any -nnll -s0 -A port \$(docker inspect --format '\''{{range \$p, \$conf := .NetworkSettings.Ports}}{{\$p}} {{end}}'\'' \$0 | awk '\''{print \$1}'\'' | cut -d '\''/'\'' -f1)"'
